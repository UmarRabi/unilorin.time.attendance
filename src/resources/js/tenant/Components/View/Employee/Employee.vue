<template>
  <div class="content-wrapper">
          ...
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb p-0 d-flex align-items-center mb-primary">
              ...
              <template v-if="this.$can('view_employees') && !ownProfile">
                ...
                <li>
                  <a
                    :href="urlGenerator(apiUrl.ALL_EMPLOYEE_URL_FRONT_END)"
                    class="btn btn-link text-primary pl-2"
                    >{{ $t("back_to_all_employees") }}</a
                  >
                </li>
              </template>
            </ol>
    ...
    <div class="user-profile mb-primary">
      <div
        class="card card-with-shadow py-5 border-0"
      ...
      >
        ...
        <div v-else class="row">
          <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-5">
            <div
              class="media border-right px-5 pr-xl-5 pl-xl-0 user-header-media align-items-center"
            >
              ...
              <div class="media-body user-info-header">
                ...
                <span
                  ...
                  :class="[
                    'badge',
                    'badge-pill',
                    'badge-' + employee.employment_status.class,
                    'user-status',
                  ]"
                >
                ...
                <p
                  ...
                  v-if="employee.profile && employee.designation"
                >
                ...
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-6">
                  <div class="media mb-4 mb-xl-5">
                    ...
                    <div class="media-body">
                      ...
                      <p
                        class="mb-0"
                        v-if="!employee.updated_salary && !employee.salary"
                      >
                        {{ $t("not_added_yet") }}
                      </p>
                      <template v-else>
                        <p class="mb-0" v-if="employee.salary">
                          {{
                            formatCurrency(
                              numberFormatter(Number(employee.salary.amount))
                            )
                          }}
                          <span class="text-primary font-size-80"
                            >(current)</span
                          >
                        </p>
                        <p
                          ...
                          v-if="
                            ...
                            isAfterNow(employee.updated_salary.start_at)
                          "
                        >
                          {{
                            formatCurrency(
                              numberFormatter(
                                Number(employee.updated_salary.amount)
                              )
                            )
                          }}
                          <span class="font-size-80"
                            ...
                            {{
                              formatDateToLocal(
                                employee.updated_salary.start_at
                              )
                            }})</span
                          >
                        </p>
                  ...
                  <div class="media mb-0 mb-xl-0">
                    ...
                    <div class="media-body">
                      ...
                      <p
                        class="mb-0"
                        v-if="employee.profile && employee.profile.joining_date"
                      >
                        {{ formatDateToLocal(employee.profile.joining_date) }}
                      </p>
                      <p class="mb-0" v-else>
                        {{ $t("not_added_yet") }}
                      </p>
...
import {
  formatCurrency,
  numberFormatter,
} from "../../../../common/Helper/Support/SettingsHelper";
...
export default {
  ...
  created() {
    this.tabs = this.ownProfile
      ? this.jobDeskTabs
      : this.employeeDetailsTabs.concat(this.jobDeskTabs);
  },
  mounted() {
  ...
  methods: {
    readURL() {
      ...
      if (this.files && this.files[0]) {
        ...
        axiosPost(`admin/auth/users/profile-picture`, formData)
          .then((response) => {
            this.$store.dispatch("getEmployeeDetails", this.employeeId);
  ...
  computed: {
    ...
    attendanceActionTitle() {
      return this.$can("update_attendance_status")
        ? this.$t("add_attendance")
        : this.$t("request_attendance");
    },
    leaveActionTitle() {
      return this.$can("assign_leaves")
        ? this.$t("assign_leave")
        : this.$t("request_leave");
    },
    ownProfile() {
      return window.user.id === this.employeeId;
    },
    joiningDateTitle() {
      return this.employee.profile?.joining_date
        ? this.$t("edit_joining_date")
        : this.$t("add_joining_date");
    },
    salaryTitle() {
      return this.employee.updated_salary
        ? this.$t("edit_salary")
        : this.$t("add_salary");
    },
    rolesNames() {
    ...
    isActionButtonHide() {
      return (
        !this.attendanceActionPermission &&
    ...
    attendanceActionPermission() {
      return (
        this.employee.employment_status &&
        this.employee.employment_status.alias !== "terminated" &&
        this.employee.status.name !== "status_inactive"
      );
    },
    ...
    terminatePermission() {
      return (
        this.employee.employment_status &&
        this.employee.status.name !== "status_invited" &&
        this.employee.employment_status.alias !== "terminated" &&
        this.$can("terminate_employees")
      );
    },
    salaryPermission() {
      return (
        this.$can("update_employee_job_history") &&
        this.employee.employment_status &&
        this.employee.employment_status.alias !== "terminated"
      );
    },
